...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($identifier === '' || $password === '') {
        ...
        $stmt = $conn->prepare('SELECT id, username, email, password, role_id, status FROM users WHERE username = ? OR email = ? LIMIT 1');
        ...
        if ($result && $result->num_rows === 1) {
            ...
            if (password_verify($password, $user['password'])) {
                if ($user['status'] === 'pending') {
                    ...
                    $_SESSION['role_id'] = $user['role_id']; // 👈 role session me save karo
